...
def do_marspose_train(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        #
        for n_iter, (img,keypoints, pid, target_cam, labels2) in enumerate(train_loader):
            optimizer.zero_grad()
            optimizer_center.zero_grad()
            img = img.to(device) # bs 3,h,w tensor
            ...
            with amp.autocast(enabled=True):
                target_cam = target_cam.view(-1)
                score, feat, _,score_vit,feat_vit = model(img, keypoints, label=target, cam_label=target_cam)
