...
class Marstest(object):
    ...
    def __init__(self, min_seq_len=0, sample_size=100):
        self._check_before_run()
    ...
    def _process_data(self, names, meta_data, home_dir=None, relabel=False, min_seq_len=0, sample_size=None):
        assert home_dir in ['bbox_train', 'bbox_test']
        num_tracklets = meta_data.shape[0] #8298
        pid_list = list(set(meta_data[:,2].tolist())) #list 625
        num_pids = len(pid_list)
        ...
        # 随机选择一定数量的 tracklets
        indices = list(range(num_tracklets))
        if sample_size is not None:
            indices = random.sample(indices, sample_size)

        for tracklet_idx in indices:
            data = meta_data[tracklet_idx,...]
            start_index, end_index, pid, camid = data #也就是mat中的四个值，分别是开始索引，结束索引，pid，camid
            if pid == -1: continue # junk images are just ignored
            assert 1 <= camid <= 6
