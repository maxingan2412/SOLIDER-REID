...
from utility import RandomIdentitySampler_mars,RandomErasing3,RandomIdentitySampler_marspose
...
def train_marspose_collate_fn(
        batch):  # batch list 32   里面是含有4个元素的tuple。 0元素： img tensor 4 3,256,128  1元素：int pid  2元素：list camid 3元素：tensor 4

    imgs,keypoints, pids, camids, a = zip(*batch)
    pids = torch.tensor(pids, dtype=torch.int64)  # tensor 32 不重复的pid是8
    ...
    return torch.stack(imgs, dim=0), torch.stack(keypoints,dim=0), pids, camids, torch.stack(a, dim=0)  # imgs tuple64  tensor 4,3,256,128   torch.stack(imgs, dim=0): tensor 64,4,3,256,128
...
#从上面改动 数据集的读取，相似的地方很多。 直接新建一个类去弄就可以了。 上面val train不同的地方只在于 sampler
...
def make_marspose_dataloader(cfg):
    Dataset_name = cfg.DATASETS.NAMES
    seq_len = cfg.DATALOADER.NUM_INSTANCE
    batchsize = cfg.SOLVER.IMS_PER_BATCH
    num_workers = cfg.DATALOADER.NUM_WORKERS
    ...
    dataset = __factory[
        Dataset_name]()  # 这里指向mars数据集，看mars类中是怎么构造出这个数据集的 再这里,获得了datasets类的实例，这个实例就是mars数据集，里面有train，query，gallery等等
    # VideoDataset_inderase 这个生成了trainset 里面有随机擦除的操作
    train_set = PoseDataset_inderase(dataset=dataset.train, seq_len=num_instances, sample='intelligent',
                                      transform=train_transforms,name = Dataset_name)  # 可以认为是把mars数据集中的train部分拿出来，然后用VideoDataset类进行处理，这里是RandomErasing3， 得到train_set
    num_classes = dataset.num_train_pids
    cam_num = dataset.num_train_cams
    view_num = dataset.num_train_vids
    ...
    # 这里该bs   823 想办法 把 q g 也弄出来batchsize 这个sampler让数据从 8298组变成了 7532组，假如说这个tracklets少于seqlen就不要了，所以变成了7532 / bs = loader_len
    train_loader = DataLoader(dataset=train_set, batch_size=batchsize,
                              sampler=RandomIdentitySampler_marspose(data_source=dataset.train, batch_size=batchsize,
                                                            num_instances=num_instances), num_workers=num_workers,
                              collate_fn=train_marspose_collate_fn)  # 这里定义了bs 这段代码使用了 PyTorch 中的 DataLoader 类，用于构建一个用于训练的数据加载器。DataLoader 提供了一种简便的方式来加载和处理训练数据，它可以在训练过程中自动进行批量化、随机化等操作。
    # q g 基本没处理  比如q 有1980长度，其中每个元素是一个tracklets，tracklets里面的图片数量就是原始的数量，大小不一，for循环出来是个四元组 img pid camid img_path
    ...
    # 新方法
    q_val_set = PoseDataset(dataset=dataset.query, seq_len=seq_len, sample='dense', transform=val_transforms)
    g_val_set = PoseDataset(dataset=dataset.gallery, seq_len=seq_len, sample='dense', transform=val_transforms)
...
class PoseDataset(Dataset):
    """Pose Person ReID Dataset. by mxa
    Note batch data has shape (batch, seq_len, channel, height, width).
    """
    sample_methods = ['evenly', 'random', 'all']
    ...
    def __getitem__(self, index):
        img_paths,keypoints, pid, camid = self.dataset[index]
        num = len(img_paths)
...
class PoseDataset_inderase(Dataset):
    """Video Person ReID Dataset.
    Note batch data has shape (batch, seq_len, channel, height, width).
    """
    sample_methods = ['evenly', 'random', 'all']

    def __init__(self, dataset, seq_len=15, sample='evenly', transform=None, name = 'marspose' ,max_length=40):
        self.dataset = dataset  # list 8298. 里面三元组 图片路径，pid，camid
        self.seq_len = seq_len  # 4
        self.sample = sample  # 'intelligent'
        self.transform = transform  # [Resize(size=[256, 128], interpolation=bicubic), RandomHorizontalFlip(p=0.5), Pad(padding=10, fill=0, padding_mode=constant), RandomCrop(size=(256, 128), padding=None), ToTensor(), Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])]
    ...
    def __getitem__(self,
                    index):  # __getitem__ 方法通过使对象支持索引操作，使得我们可以按照序列的方式访问自定义类的元素,__是双下划线（__）开头和结尾的函数在Python中被称为"魔法方法"（magic methods）或"特殊方法"（special methods）。这些方法是Python类中的特殊函数，用于定义类的行为，例如初始化、比较、运算符重载等。这些魔法方法会在特定的上下文中被自动调用，而不需要直接调用它们。
        img_paths,keypoint_paths, pid, camid = self.dataset[index]  # 这里拿到的是随机的一个tracklets 再一个tracklets中只有一个pid和camid。
        num = len(img_paths)
        if self.sample != "intelligent":
            frame_indices = range(num)
            rand_end = max(0, len(frame_indices) - self.seq_len - 1)
        ...
        if not hasattr(self, 'erase'):
            ...
            for index in indices:  # 便利index，拿到这些index位置的图片
                index = int(index)
                img_path = img_paths[index]
                keypoint_path = keypoint_paths[index]
                img = read_image(img_path)

                if self.name == 'marspose':
                    kps = read_keypoints_mars(keypoint_path)
                elif self.name == 'posetrackreid':
                    kps = read_keypoints_posetrackreid(keypoint_path)
                ...
                if self.transform is not None:
                    img = self.transform(img)  # 这里已经把图片用归一化等东西 tensro已经是比较小的数，比如-1 0.5之类，同时这里img已经是 tensor 3 256 128
                    _,h,w = img.shape
                    kps = normalize_pose(kps, True,w, h)
            ...
            labels = torch.tensor(labels)
            return imgs,keypoints, pid, targt_cam, labels
        else:
            # 当self.erase存在时执行的代码,也是VID的代码
            imgs = []
            labels = []
